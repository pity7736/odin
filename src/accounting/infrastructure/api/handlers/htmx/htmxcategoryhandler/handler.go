...
func (self *htmxCategoryHandler) HandleOneResponse(category *categorymodel.Category) error {
	isFirst, _ := strconv.ParseBool(self.ctx.FormValue("first", "false"))
	if isFirst {
		self.ctx.Set("HX-Refresh", "true")
	}
	return self.ctx.Render("category", category, "")
}

func (self *htmxCategoryHandler) HandleManyResponse(categories []*categorymodel.Category) error {
	return self.ctx.Render("categories", data{Categories: categories})
}
