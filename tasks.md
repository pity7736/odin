...
- [x] CLI + login verification: build a small CLI (`cmd/odin-cli`) that does client-side crypto (Argon2id key derivation), derives the auth hash for the seeded user, calls the login endpoint, and verifies it gets back the encrypted master key and key params. First real proof the crypto handshake works. The CLI grows with each subsequent task.
- [x] Registration: accept email, auth_hash, encrypted_master_key, and key_params from the client. Server bcrypt-hashes the auth_hash and persists the user. Remove seed data — the CLI registers its own users from this point on. Full SDD cycle.
- [ ] Prelogin key params (LOW PRIORITY — multi-device only, deferred): unauthenticated endpoint that returns a user's key derivation params (salt + Argon2id params) by email, so a client that doesn't already have the salt (second device, reinstall) can derive the auth_hash before logging in. Not needed for single-device use where the client already stored its salt at registration. Returns deterministic dummy params for unknown emails to avoid email enumeration; never returns the encrypted master key. Full SDD cycle.
- [x] Create encrypted chunk: the CLI encrypts plaintext with AES-256-GCM using the master key, sends the blob to a new server endpoint, reads it back, decrypts, and verifies the plaintext matches. Server-side: accept a client-encrypted blob (ciphertext + IV + auth tag) with a client-generated ID, persist it. This is where the EncryptedChunk entity gets designed — you can't create what you haven't defined. Full SDD cycle.
- [x] Read encrypted chunk: retrieve a single encrypted chunk by ID for its owner. Server returns the opaque blob as-is — no decryption, no interpretation. CLI decrypts and verifies the round-trip.
- [ ] Get all chunks: retrieve every encrypted chunk owned by the requesting user. Server returns the opaque blobs as-is — no decryption, no interpretation; scoped to the owner so no other user's chunks are ever returned. CLI fetches the collection, decrypts each, and verifies them against what it stored. Full SDD cycle.
...
- [x] Bug: requesting a non-existent URL returns 500 instead of 404. The global error handler falls through for non-odin errors (including unmatched routes) and yields an empty 500; unmatched routes should return 404.
- [x] Fix repository fetch contract: a repository fetch should signal absence with a `NotFound` error, not `(nil, nil)`. Today `UserRepository.GetByEmail` returns `(nil, nil)` for a missing user, forcing callers to nil-check and leaving a footgun (a future adapter returning `NotFound` would break them). Change `GetByEmail` (and any future `GetBy*` fetch) to return an `odinerrors` `NotFound` on absence, and update `sessionstarter`/login to treat `NotFound` as wrong-credentials. Existence checks use `Exists` (already done for chunks).
